...
if ($_SERVER ['REQUEST_METHOD'] == 'POST') {
	...
	$comentarios = "* Se ha grabado la Tarea (" . $dias [date ( "w" )] . ", " . date ( "d" ) . " del " . date ( "m" ) . " de " . date ( "Y" ) . " " . date ( "H" ) . ":" . date ( "i" ) . ") :";
	$comentarios = $comentarios . "\r\n" . "     " . "- Estado: " . $txt_estado [1] . " - Modificada por usuario: " . $_SESSION ['usuario'];
	$comentarios = $comentarios . "\r\n" . "     " . "- Modificada por usuario: " . $_SESSION ['usuario'];
	$comentarios = $comentarios . "\r\n" . "     " . "- Actuacion Jazztel: " . $act_jazztel;
	$comentarios = $comentarios . "\r\n" . "     " . "- Actividad / Subactividad: " . $descr_activ . " / " . $descr_subactiv;
	$comentarios = $comentarios . "\r\n" . "     " . "- Escalado: " . $ticket_escalado;
	$comentarios = $comentarios . "\r\n" . "     " . "- Ticket Remedy: " . $ticket_remedy;
	$comentarios = $comentarios . "\r\n" . "     " . "- Ticket Oceane: " . $ticket_oceane;
	$comentarios = $comentarios . "\r\n" . "     " . "- SUC: " . $suc;
	$comentarios = $comentarios . "\r\n" . "     " . "- TP: " . $ticket_tp;
	$comentarios = $comentarios . "\r\n" . "     " . "- Cto nueva: " . $_POST ['CTO_NUEVA'];
	$comentarios = $comentarios . "\r\n" . "     " . "- Editor de incidencia: " . $_POST ['COMENTARIOS2'];
	$comentarios = $comentarios . "\r\n\r\n" . $_POST ['COMENTARIOS'];
	...
	if (isset ( $_POST ['cambiarEstado'] )) {
		// echo ($_POST['ESTADO']); //Ejemplpo: 1-Pendiente
		$mensaje = cambiarEstado ( $conn, $id, $id_usuario, $comentarios );
		// echo $mensaje;exit();
	...
	// echo ("Nuevo estado: ".$nuevoEstado[0]); exit();
	...
	// echo $nuevoEstado[0]; echo $nuevoEstado[1]; exit();
	if ($ctrl_envioMail) {
		...
		$sql_u = "select *, (select mail from INV_tbUSUARIOS where id_usu=tbu.idResponsable) as mail_responsable, INV_tbGrupos.grupo as des_grupo," . "(select mail from INV_tbUSUARIOS where id_usu=" . $id_usuorigen . ") as mail_usuorigen from INV_tbUSUARIOS as tbu, INV_tbGrupos where usuario='" . $_SESSION ['usuario'] . "' and tbu.idGrupo=INV_tbGrupos.id_grupo";
		// echo $sql_u;exit();
		$resultado_u = sqlsrv_query ( $conn, $sql_u );
		$grupo_u = "";
		$id_grupo_u = "";
		$mail_usuorigen = "";
		$mail_responsable = "";
		$telefono_usuario = "";
		if ($resultado_u === false) {
			// Nada
		} else {
			$registro_u = sqlsrv_fetch_array ( $resultado_u );
			$grupo_u = $registro_u ['grupo'];
			$id_grupo_u = $registro_u ['id_grupo'];
			$des_grupo_u = utf8_decode ( $registro_u ['des_grupo'] );
			
			if ($registro_u ['' . 'mail_usuorigen'] != "" && $registro_u ['mail_usuorigen'] != NULL && $registro_u ['mail_usuorigen'] != "NULL") {
				$mail_usuorigen = $registro_u ['mail_usuorigen'];
			}
			if ($registro_u ['mail_responsable'] != "" && $registro_u ['mail_responsable'] != NULL && $registro_u ['mail_responsable'] != "NULL") {
				$mail_responsable = $registro_u ['mail_responsable'];
			}
			if ($registro_u ['Telefono'] != "" && $registro_u ['Telefono'] != NULL && $registro_u ['Telefono'] != "NULL") {
				$telefono_usuario = $registro_u ['Telefono'];
			}
		}
		sqlsrv_free_stmt ( $registro_u );
		
		// DESTINATARIOS
		//
		// usuario que realiza la petición (usuario origen)
		$mailUsu = $_SESSION ['mail'];
		$nombreUsu = $_SESSION ['nombreUsuOr'];
		
		// ST
		$mailST = 'invfisico.ftth@orange.com';
		$nombreST = 'Servicios Tecnicos I.Fisico';
		
		// ST inv fco
		$mailSTfco = 'invfisico.ftth@orange.com';
		$nombreSTfco = 'Servicios Tecnicos I.Fisico';
		// echo $id_grupo_u; exit();
		// Comprobar tipo de inventario y enviar desde ese
		if ($id_grupo_u == "1") {
		...
		$mailFrancF = 'francisco.flores@orange.com';
		$nombreFrancF = 'Francisco Flores Diéguez';
		
		// SSR
		$mailSSR = 'ssrftth@orange.com';
		$nombreSSR = 'SSR';
		
		// Ana Mercedes siempre en copia
		$mailAnaM = 'anamercedes.roldan@orange.com';
		$nombreAnaM = 'Ana Mercedes Roldán';
		
		// Javier Mora
		$mailJavierMora = 'javier.mora@orange.com';
		$nombreJavierMora = 'Javier Mora Alcázar';
		
		// Álvaro Martin Barrena
		$mailAlvaroMartin = 'alvaro.martinb@orange.com';
		$nombreAlvaroMartin = 'Álvaro Martin Barrena';
		
		// Álvaro Martin Barrena
		$mailGerardoAlberto = 'gdem.ext@orange.com';
		...
		// Obtener mail del responsable USUORIGEN
		sqlsrv_free_stmt ( $stmt );
		$tsql = "SELECT INV_tbUSUARIOS.mail from INV_tbUSUARIOS where INV_tbUSUARIOS.id_usu in(SELECT dbo.INV_tbUSUARIOS.idResponsable FROM dbo.INV_tbUSUARIOS Where mail='" . $registro_u ['mail_usuorigen'] . "')";
		$stmt = sqlsrv_query ( $conn, $tsql ) or die ( "Fallo en la consulta responsable USUORIGEN" );
		$rows = sqlsrv_has_rows ( $stmt );
		if ($rows === true) {
			while ( $row = sqlsrv_fetch_array ( $stmt ) ) {
				if ($row ["mail"] != '') {
					$mailResponsableUsuOrigen = $row ["mail"];
				}
			}
		} else {
			$mailResponsableUsuOrigen = '';
		}

		// Obtener mail del Regional
		sqlsrv_free_stmt ( $stmt );
		$tsql = "SELECT inv_provincias.Regional from inv_provincias where inv_provincias.Descripcion='" . $provincia .	"'";
		//echo $provincia;
		$stmt = sqlsrv_query ( $conn, $tsql ) or die ( "Fallo en la consulta del Regional" );
		$rows = sqlsrv_has_rows ( $stmt );
		if ($rows === true) {
			while ( $row = sqlsrv_fetch_array ( $stmt ) ) {
				if ($row ["Regional"] != '') {
					$mailRegional = $row ["Regional"];
				}
			}
		} else {
			$mailRegional = '';
		...
		$cuerpo = '
                            ...
                            ';
		// echo ($cuerpo);exit();
		if (isset ( $selected_cambioEst ) && $selected_cambioEst != "") {
			...
			switch ($selected_cambioEst) {
				...
				case 3 : // Pte contestación
					$estado = 'Quedamos a la espera de vuestra contestación, para ello es necesario introducir en la aplicación web
                                                            el número de referencia asignado a esta tarea.';
					break;
				...
				case 8 : // En construccion
					...
					break;
				case 9 : // Pendiente de SUC
					$estado = 'Pendiente de SUC';
					break;
				case 10 : // Pendiente de aprobación Presupuesto
					$estado = 'Pendiente de aprobación Presupuesto';
					break;
				case 11 : // Pendiente de Ingeniería
			...
			$cuerpo = $cuerpo . '
                                            <p><span style="font-style: italic">Estado: </span><span style="font-weight: bold">' . $estado . '</span></br>								
                            ';
		}
		...
		if (isset ( $_REQUEST ["SUC"] ) && $_REQUEST ["SUC"] != "") {
			$cuerpo = $cuerpo . '
                                            <span style="font-style: italic">SUC: </span><span style="font-weight: bold">' . $_REQUEST ["SUC"] . '</span></br>								
                            ';
		}
		// echo ($cuerpo);exit();
	...
	//	}
		...
		$cuerpo = $cuerpo . '<p>Un saludo</p>
                                            ...
                                            <p>
                                                ...
                                                <span style="font-weight: bold;color:#c45911">' . $grupo_u . '</span></br>
                                            </p>
                                            <p style="color:#8b8b8b">
                                                C/ Campezo, 1 </br>
                                                Parque Empresarial Las Mercedes Edif. 3</br>
                                                28022 Madrid</br>
                                                Tel. fijo: ' . $telefono_usuario . '</br>
		...
		// asunto
		// cabecera
		$cabeceraMail = '[' . $des_grupo_u . '] REF: ' . $ref;
		...
		if (isset ( $_REQUEST ["SUC"] ) && $_REQUEST ["SUC"] != "") {
			$cabeceraMail = $cabeceraMail . ' - SUC: ' . $_REQUEST ["SUC"];
		}
		...
		// echo ('***************************************'.$selected_cambioEst); EXIT();
		/*
		 * $nuevoEstado= split("-", $selected_cambioEst);
		 * echo ('************************ '.$$nuevoEstado); exit();
		 */
		
		switch ($selected_cambioEst) {
			
			case 1 : // pendiente
				$cabeceraMail = $cabeceraMail . '- Solicitud pendiente';
				break;
			
			case 4 : // cerrada
				$cabeceraMail = $cabeceraMail . '- Solicitud cerrada';
				break;
			case 3 : // Pte contestación
		...
		if ($mailUsu != "" && $mailUsu != NULL && $mailUsu != "NULL") {
			$destinatarios_mail = $mailST . ", " . $mailUsu . ", " . $mail_responsable . ", " . $mail_usuorigen . ", " . $mailRegional . ", " . $mailResponsableUsuOrigen;
		} else {
			$destinatarios_mail = $mailST . ", " . $mail_responsable . ", " . $mailRegional .", " . $mail_usuorigen;
		}
		$destinatarios_mail = str_replace ( ", , , ,", ",", $destinatarios_mail );
		$destinatarios_mail = str_replace ( ", , ,", ",", $destinatarios_mail );
		$destinatarios_mail = str_replace ( ", ,", ",", $destinatarios_mail );
		if (isset ( $mailporActiv ) && $mailporActiv != "") {
			$destinatarios_mail = $destinatarios_mail . ", $mailporActiv";
		}
		if (isset ( $mailporActiv2 ) && $mailporActiv != "") {
			$destinatarios_mail = $destinatarios_mail . ", $mailporActiv2";
		}
		if (isset ( $mailResponsable ) && $mailporActiv != "") {
			$destinatarios_mail = $destinatarios_mail . ", $mailResponsable";
		}
...
if ($id != '') {
	...
	if ((($registro ['REF_ASOCIADA'] != '') && ! empty ( $registro ['REF_ASOCIADA'] )) || ($registro ['INCIDENCIA'] != '' && ! empty ( $registro ['INCIDENCIA'] ))) {
		$tsql = "SELECT *
				FROM INV_TBTAREAS AS A
				LEFT JOIN INV_VIEW_DATOS_TODO AS B ON B.ID_TAREA = A.ID";
		
		if (($registro ['REF_ASOCIADA'] != '') && ! empty ( $registro ['REF_ASOCIADA'] )) {
			if ($registro ['INCIDENCIA'] != '' && ! empty ( $registro ['INCIDENCIA'] )) {
		...
		<div id="content" class="span12">
			...
			<form method="post" action="modificarTarea.php" role="form" enctype="multipart/form-data">
				...
				<div style="padding-left:5px;" class="row-fluid yellow">
					<div class="span2" ontablet="span4" ondesktop="span2">
						<div class="control-group form-group">
							<div class="controls">
								<strong>PROVINCIA: </strong><br><input readonly="true" type="text" class="form-control input uneditable-input" name="PROVINCIA" value="<?php
					...
					<div class="span2" ontablet="span4" ondesktop="span2">
						...
						<div class="control-group form-group">
							<div class="controls">
								<strong>REGIÓN: </strong><br><input readonly="true" type="text" class="form-control input uneditable-input" name="REGION" value="<?php
					...
					<div class="span2" ontablet="span4" ondesktop="span2">
						...
						<div class="control-group form-group">
							<div class="controls">
								<strong>GESTOR: </strong><br><input readonly="true" type="text" class="form-control input uneditable-input" name="GESTOR" value="<?php
					...
					<div class="span2" ontablet="span4" ondesktop="span2">
						<div class="control-group form-group">
							<div class="controls">
								...
								<strong>USUORIGEN: </strong><br><input readonly="true" type="text" class="form-control input uneditable-input" name="USUORIGEN" value="<?php
								
								echo get_nombreFromId ( $id_usuorigen );
								?>">
								<?php
								
								echo '<input readonly="true" class="hidden" name="ID_USUORIGEN" value="' . $id_usuorigen . '">';
								?>
					...
					<div class="span2" ontablet="span4" ondesktop="span2">
						<div class="control-group form-group">
							<div class="controls">
								<strong>GRUPO: </strong><br><input readonly="true" type="text" class="form-control input uneditable-input" name="GRUPO" value="<?php
					...
					<div class="span2" ontablet="span4" ondesktop="span2">
						<div class="control-group form-group">
							<div class="controls">
								<strong>GRUPO_ESCALADO: </strong><br><input readonly="true" type="text" class="form-control input uneditable-input" name="grupo_escalado" value="<?php
				...
				<div style="padding-left:5px;" class="row-fluid yellow">
					...
					<div class="control-group form-group span2">
						<div class="controls">						
							<strong>CABECERA: <br /></strong>	
								<input type="text" readonly id="cabecera" name="cabecera" value="<?php
								
								echo $cabecera;
								?>"/>
								<input class="hidden" id="codCab" name="codCab" value="<?php
					...
					<div class="control-group form-group span2">
						<div class="controls">
							<strong>ACT. JAZZTEL: <br /></strong>
								<input type="text" class="form-control input"  name="actJazz" value="<?php
					...
					<div class="control-group form-group span2">
						<div class="controls">
							<strong>ACT. TESA: <br /></strong>
								<input type="text" readonly id="actTesa" name="actTesa" value="<?php
					...
					<div class="control-group form-group span2">
						<div class="controls">
							<strong>ID ACTUACIÓN: <br /></strong>
								<input type="text" readonly id="idAct" name="idAct" value="<?php
					...
					<div class="control-group form-group span2">
						...
						<div class="controls">
							<strong>ID_FDTT: <br /></strong>
								<input type="text" readonly id="idFDTT" name="idFDTT" value="<?php
					...
					<div class="control-group form-group span2">
						<div class="controls">
							<strong>TÉCNICO: </strong><br><?php
							
							echo '<input readonly="true" type="text" class="form-control input uneditable-input" name="tecnico" value="' . (($tecnico == '') ? get_nombre ( $_SESSION ['usuario'] ) : $tecnico) . '">';
							?>
						</div>
					</div>			
					<div class="control-group form-group">
						<div class="controls">
							<input readonly id="idGD" class="hidden" name="idGD" value="<?php
							
							echo $id_gd;
							?>"/>
							<?php
							
							echo '<input readoly="true" class="hidden" name="id_tecnico" value="' . (($tecnico == '') ? get_idUsu ( $_SESSION ['usuario'] ) : get_idFromNombre ( $tecnico )) . '">';
							?>
						</div>
					</div>		
				...
				<div style="padding-left:5px;" class="row-fluid yellow">
					...
					<div class="control-group form-group span2">
						<div class="controls">
							<strong>REFERENCIA ASOCIADA: <br /></strong>
								<input type="text" readonly id="refAsoc" name="refAsoc" value="<?php
					...
					<div class="control-group form-group span2">
						<div class="controls">
							<strong>HUELLA: <br /></strong>
								<input type="text" readonly id="huella" name="huella" value="<?php
					...
					<div class="control-group form-group span2">
						<div class="controls">
							<strong>REF: </strong><br><input readonly="true" type="text" class="form-control input uneditable-input" name="ref" value="<?php
					...
					<div class="control-group form-group span2">
						<div class="controls">
							<strong>ID: </strong><br><input readonly="true" type="text" class="form-control input uneditable-input" name="id" value="<?php
					...
					<div class="control-group form-group span2">
						<div class="controls">
							<strong>PRIORIDAD: </strong><br><input readonly="true" type="text" class="form-control input uneditable-input" name="prioridad" value="<?php
					...
					<div class="control-group form-group span2">
						<div class="controls">
							<strong>EEMM: </strong><br><input readonly="true" type="text" class="form-control input uneditable-input" name="EEMM" value="<?php
				...
				<div style="padding-left:5px;" class="row-fluid yellow">
					<div class="span2" ontablet="span4" ondesktop="span2">
						<div class="control-group form-group">
							<div class="controls">
								...
								<a title="Detalle datos ID_MAPEO" class="btn btn-mini buscar_mapeo" data-toggle="modal" data-target="#viewModalMP" data-id="<?php
								
								echo $id_mapeo;
								?>">		
									<i class="halflings-icon white eye-open"></i>  
								</a>
								<input readonly="true" type="text" class="form-control input uneditable-input" name="id_mapeo" value="<?php
					...
					<div class="span2" ontablet="span4" ondesktop="span2">
						...
						<div class="control-group form-group">
							<div class="controls">
								<strong>ID_TIPO_ENTRADA: </strong><br><input readonly="true" type="text" class="form-control input uneditable-input" name="id_tipo_entrada" value="<?php
								
								echo $id_tipo_entrada;
								?>">
							</div>
						</div>					
					...
					<div class="span2" ontablet="span4" ondesktop="span2">
						<div class="control-group form-group">
							<div class="controls">
								...
								<strong>TIPOLOGIA_INICIAL: </strong><br><input readonly="true" type="text" class="form-control input uneditable-input" name="tipologia_inicial" value="<?php
								
								echo $tipologia_inicial;
								?>">
					...
					<div class="span2" ontablet="span4" ondesktop="span2">
						<div class="control-group form-group">
							<div class="controls">
								<strong>TIPO_INCIDENCIA: </strong><br><input readonly="true" type="text" class="form-control input uneditable-input" name="tipo_incidencia" value="<?php
								
								echo $tipo_incidencia;
								?>">
							</div>
						</div>											
					...
					<div class="span2" ontablet="span4" ondesktop="span2">
						<div class="control-group form-group">
							<div class="controls">
								<strong>TIPO_CLIENTE: </strong><br><input readonly="true" type="text" class="form-control input uneditable-input" name="tipo_cliente" value="<?php
				...
				<div class="row-fluid" style="margin-bottom:10px;">
					...
					<div class="box-header">
						...
						<h2><i class="halflings-icon list"></i><span class="break"></span>Detalle tarea</h2><a style="margin-left:20px;" title="Historia Tarea" class="btn btn-mini btn-primary buscar_historia" data-toggle="modal" data-target="#historiaModal" data-id="<?php
				...
				<div class="row-fluid">
								...
								<div class="controls">
									...
									?>
                                                ...
                                                <div class="control-group">
                                                          <div class="control-group form-group">
                                                                            <div class="controls">
                                                                                    <strong>ESCALADO: </strong><br>
                                                                                    <select name="TICKET_ESCALADO">
                                                                                      <option value=""></option>
                                                                                      <option value="GAMMA" <?php
																																																																																						
																																																																																						if ($ticket_escalado === "GAMMA") {
																																																																																							?> selected <?php
																																																																																						}
																																																																																						?>>GAMMA</option>
                                                                                      <option value="Reg_Cataluña" <?php
																																																																																						
																																																																																						if ($ticket_escalado === "Reg_Cataluña") {
																																																																																							?> selected <?php
																																																																																						}
																																																																																						?>>Reg_Cataluña</option>
                                                                                      <option value="Reg_Centro" <?php
																																																																																						
																																																																																						if ($ticket_escalado === "Reg_Centro") {
																																																																																							?> selected <?php
																																																																																						}
																																																																																						?> >Reg_Centro</option>
                                                                                      <option value="Reg_Levante" <?php
																																																																																						
																																																																																						if ($ticket_escalado === "Reg_Levante") {
																																																																																							?> selected <?php
																																																																																						}
																																																																																						?>>Reg_Levante</option>
                                                                                      <option value="Reg_Noroeste" <?php
																																																																																						
																																																																																						if ($ticket_escalado === "Reg_Noroeste") {
																																																																																							?> selected <?php
																																																																																						}
																																																																																						?>>Reg_Noroeste</option>
                                                                                      <option value="IBM" <?php
																																																																																						
																																																																																						if ($ticket_escalado === "IBM") {
																																																																																							?> selected <?php
																																																																																						}
																																																																																						?>>IBM</option>
                                                                                      <option value="Ingenieria" <?php
																																																																																						
																																																																																						if ($ticket_escalado === "Ingenieria") {
																																																																																							?> selected <?php
																																																																																						}
																																																																																						?>>Ingenieria</option>
                                                                                      <option value="Reg_Oriental" <?php
																																																																																						
																																																																																						if ($ticket_escalado === "Reg_Oriental") {
																																																																																							?> selected <?php
																																																																																						}
																																																																																						?>>Reg_Oriental</option>
                                                                                      <option value="Reg_Occidental" <?php
																																																																																						
																																																																																						if ($ticket_escalado === "Reg_Occidental") {
																																																																																							?> selected <?php
																																																																																						}
																																																																																						?>>Reg_Occidental</option>
                                                                                      <option value="ACCENTURE" <?php
																																																																																						
																																																																																						if ($ticket_escalado === "ACCENTURE") {
																																																																																							?> selected <?php
																																																																																						}
																																																																																						?>>ACCENTURE</option>
                                                                                      <option value="EEMM" <?php
																																																																																						
																																																																																						if ($ticket_escalado === "EEMM") {
																																																																																							?> selected <?php
																																																																																						}
																																																																																						?>>EEMM</option>
                                                                                      <option value="SSTT" <?php
																																																																																						
																																																																																						if ($ticket_escalado === "SSTT") {
																																																																																							?> selected <?php
																																																																																						}
																																																																																						?>>SSTT</option>
                                                                                      <option value="Telefonica" <?php
																																																																																						
																																																																																						if ($ticket_escalado === "Telefonica") {
																																																																																							?> selected <?php
																																																																																						}
																																																																																						?>>Telefonica</option>
                                                                                    </select>
                                                                    ...
                                                                    <div class="control-group form-group">
                                                                            <div class="controls">
                                                                                    <strong>SUC: </strong><br><input type="text" class="form-control input" name="SUC" value="<?php
						...
						<div class="control-group">
							<div class="controls">
								...
								if ($_SERVER ['REQUEST_METHOD'] == 'POST' && isset ( $_POST ['modificar'] ) && isset ( $_POST ['CTOgesc'] )) {
									...
									<div>
										<label class="radio">
											<input type="radio" name="CTOgesc" id="radioCTO" value="CTO" <?php
									
									echo (($_POST ['CTOgesc'] == 'CTO') ? ' checked' : '')?> >
												CTOS
										</label>
										<div style="clear:both"></div>
										<label class="radio">
											<input type="radio" name="CTOgesc" id="radioGescales" value="GESC" <?php
									
									echo (($_POST ['CTOgesc'] == 'GESC') ? ' checked' : '')?> >
												GESCALES
										</label>
									</div>
								<?php
					...
					<div class="span3">
						<div class="control-group form-group">
							<div class="controls">
								<strong>TICKET REMEDY: </strong><br><input type="text" class="form-control input" name="INCIDENCIA" value="<?php
						...
						<div class="control-group form-group">
							<div class="controls">
								<strong>TICKET_OCEANE: </strong><br><input type="text" class="form-control input" name="TICKET_OCEANE" value="<?php
						...
						<div class="control-group form-group">
							<div class="controls">
								<strong>TP: </strong><br><input type="text" class="form-control input" name="TP" value="<?php
						...
						<div class="control-group form-group">
							<div class="controls">
								<strong>CTO_NUEVA: </strong><br><input type="text" class="form-control input" name="CTO_NUEVA" value="<?php
...
<!--                                                                                                            <div id="ctos_lateral" >-->
														...
														while ( $cto = sqlsrv_fetch_array ( $ctosActuacion ) ) {
															...
															if ($bloqueo) {
																...
																$tsql7 = $tsql7 . " WHERE(INV_tbTareas_Bloqueos.Tipo_Afectacion = 1) AND (INV_tbTareas.idEst != 6)AND ";
																$tsql7 = $tsql7 . " (INV_tbSubactividad.id_Actividad = 4)  AND INV_tbTareas_CTO.COD_CTO='" . $cto ['COD_CTO'] . "'";
																...
																if ($desbloqueo) {
																	...
																	$tsql7 = $tsql7 . " WHERE(INV_tbTareas_Desbloqueos.Tipo_Afectacion = 1) AND (INV_tbTareas.idEst != 5) AND";
																	$tsql7 = $tsql7 . " (INV_tbSubactividad.id_Actividad = 5)  AND INV_tbTareas_CTO.COD_CTO='" . $cto ['COD_CTO'] . "'";
																	$stmt7 = sqlsrv_query ( $conn, $tsql7 ) or die ( "Error al ejecutar consulta: " . $tsql7 );
																	$rows7 = sqlsrv_has_rows ( $stmt7 );
...
<!--						  							</div>-->
							                ...
							                <div class="control-group">
							                    <div class="controls">
							                    	<button id="cerrarCtos" onClick="verBoton   Gescales(); return false" type="button" class="btn btn-default" data-dismiss="modal">Aceptar</button>
							                    	<?php
						...
						if ($_SERVER ['REQUEST_METHOD'] == 'POST' && isset ( $_POST ['modificar'] ) && isset ( $_POST ['MARCAR'] ) && ($bloqueo || $desbloqueo)) {
							?>
							<div class="box-header" data-original-title>
					...
					<div class="span2">
						...
						<div class="control-group form-group">
							<div class="controls">
								<strong>FECHA_INICIO: </strong><br><input readonly="true" type="text" class="form-control input uneditable-input" name="FECHA_INICIO" value="<?php
								
								echo date_format ( $registro ['FECHA_INICIO'], 'Y-m-d H:i:s' );
								?>">
							</div>
						</div>				
						<div class="control-group form-group">
							<div class="controls">
								<strong>FECHA_RESOL: </strong><br><input readonly="true" type="text" class="form-control input uneditable-input" name="FECHA_RESOL" value="<?php
								
								echo date_format ( $registro ['FECHA_RESOL'], 'Y-m-d H:i:s' );
								?>">
							</div>
						</div>	
						<div class="control-group form-group">
							<div class="controls">
								<strong>FECHA_REGISTRO: </strong><br><input readonly="true" type="text" class="form-control input uneditable-input" name="FECHA_REGISTRO" value="<?php
								
								echo date_format ( $registro ['FECHA_REGISTRO'], 'Y-m-d H:i:s' );
								?>">
							</div>	
						</div>														
				...
				<div class="row-fluid">
					...
					<div class="form-group span8">
							<strong >Historial Incidencia: </strong><br>
							<textarea rows="5" name="COMENTARIOS" class="form-control" name="COMENTARIOS"><?php
				...
				<div class="row-fluid">
					...
					<div class="form-group span8">
							<strong >Editor de incidencia: </strong><br>
							<textarea rows="5" name="COMENTARIOS2" class="form-control" name="COMENTARIOS2"><?php
							...
							<table class="table table-striped table-bordered bootstrap-datatable datatable">
							  ...
							  <tbody>
		                         ...
		                         <?php
																											...
																											while ( $lineaAsoc = sqlsrv_fetch_array ( $resultado ) ) {
																												?>
										<td class="center"><?php
																												
																												echo $lineaAsoc ['ID_TAREA'];
																												?></td>
										<td class="center"><?php
																												
																												echo $lineaAsoc ['REMEDY'];
																												?></td>
										<td class="center"><?php
																												
																												echo $lineaAsoc ['REF_TBTAREA'];
																												?></td>
										<td class="center"><?php
																												
																												echo $lineaAsoc ['OCEANE_TBTAREA'];
																												?></td>
										<td class="center"><?php
																												
																												echo $lineaAsoc ['ESCALADO_TBTAREA'];
																												?></td>
										<td class="center"><?php
																												
																												echo $lineaAsoc ['ACTIVIDAD'];
																												?></td>
										<td class="center"><?php
																												
																												echo $lineaAsoc ['SUBACTIVIDAD'];
																												?></td>
										<td class="center"><?php
																												
																												echo $lineaAsoc ['USUORIGEN'];
																												?></td>
										<td class="center"><?php
																												
																												echo $lineaAsoc ['TECNICO'];
																												?></td>
										<td class="center"><?php
																												
																												echo date_format ( $lineaAsoc ['FECHA_REGISTRO'], 'Y-m-d H:i:s' );
																												?></td>
										<td class="center"><?php
																												
																												echo date_format ( $lineaAsoc ['FECHA_INICIO'], 'Y-m-d H:i:s' );
																												?></td>
										<td class="center"><?php
																												
																												echo date_format ( $lineaAsoc ['FECHA_RESOL'], 'Y-m-d H:i:s' );
																												?></td>
										<td class="center"><?php
																												
																												echo $lineaAsoc ['Estado'];
																												?></td>
										<td class="center"><?php
																												
																												echo $lineaAsoc ['PRIORIDAD'];
																												?></td>
										<td class="center">
											<a title="Gestionar" class="btn btn-danger btn-mini gestionar_tarea" href="<?php
																												
																												echo 'modificarTarea.php?id=' . $lineaAsoc ['ID_TAREA'];
																												?>">		
